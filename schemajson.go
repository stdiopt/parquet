...
func NewSchemaHandlerFromJSON(str string) (sh *SchemaHandler, err error) {
	...
	if err := json.Unmarshal([]byte(str), schema); err != nil {
		return nil, fmt.Errorf("error in unmarshalling json schema string: %w", err)
	}
	...
	for len(stack) > 0 {
		...
		default: // normal variable
			schema, err := NewSchemaElementFromTagMap(info)
			if err != nil {
